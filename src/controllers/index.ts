...
export interface IBaseController<T, CreateDto, UpdateDto, ResponseDto> {
  create(createDto: CreateDto): Promise<ResponseDto<ResponseDto>>;
  findAll(
    searchDto?: any,
  ): Promise<ResponseDto<{ data: ResponseDto[]; total: number }>>;
  findById(id: number): Promise<ResponseDto<ResponseDto>>;
  update(id: number, updateDto: UpdateDto): Promise<ResponseDto<ResponseDto>>;
  delete(id: number): Promise<ResponseDto<{ message: string }>>;
}

export interface IPaginatedController<T, SearchDto, ResponseDto> {
...
export interface IUserSpecificController<T, CreateDto, UpdateDto, ResponseDto> {
  createForUser(
    userId: number,
    createDto: CreateDto,
