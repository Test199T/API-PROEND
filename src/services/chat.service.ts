    ...
    if (imageUrl && analyzeImage) {
      try {
        ...
        if (analysisType === 'direct' || instruction.includes('ตอบเฉพาะสิ่งที่เห็น')) {
          // การตอบแบบตรงๆ - ไม่วิเคราะห์เชิงลึก
          const analysis = await this.foodImageService.analyzeFoodImage(
            imageUrl,
            'uploaded-image',
          );

          aiMessageText = `ฉันเห็นในรูปภาพว่าเป็น ${analysis.food_name} ${instruction ? `\n\n${instruction}` : ''}`;
        } else {
          // การวิเคราะห์แบบปกติ (เชิงลึก)
          const analysis = await this.foodImageService.analyzeFoodImage(
            imageUrl,
            'uploaded-image',
          );

          aiMessageText = `🧠 **การวิเคราะห์ภาพอาหาร (Image Analysis Layer)**

🍽️ **การตรวจจับอาหาร**: ตรวจพบ ${analysis.food_name} และส่วนประกอบอื่นๆ ในภาพ
📛 **การจำแนกประเภทอาหาร**: ${analysis.food_name} - อาหาร${analysis.meal_type || 'ทั่วไป'}
📏 **การประมาณปริมาณ**: ประมาณ ${analysis.quantity || '1'} ${analysis.unit || 'จาน'}
🎨 **การแยกส่วนผสม**: ${analysis.ingredients || 'ประกอบด้วยส่วนผสมหลักตามมาตรฐาน'}
📸 **บริบทภาพ**: ${analysis.context || 'เหมาะสำหรับมื้ออาหารหลัก'}

🧩 **การวิเคราะห์โภชนาการ (Nutritional Analysis Layer)**
...
⚙️ **การประมวลผลและวิเคราะห์ต่อยอด (Post-Processing Layer)**

📈 **การวิเคราะห์แนวโน้ม**: จากข้อมูลการบริโภคของคุณ แคลอรี่ในมื้อนี้${analysis.nutrition.calories > 600 ? 'ค่อนข้างสูง' : 'อยู่ในระดับปานกลาง'} ควรปรับสมดุลกับมื้ออื่นๆ

🎯 **การติดตามเป้าหมาย**: ${analysis.nutrition.calories < 500 ? 'มื้อนี้ช่วยสนับสนุนเป้าหมายการลดน้ำหนักได้ดี' : 'ควรควบคุมปริมาณในมื้อถัดไปเพื่อรักษาสมดุล'}

💡 **คำแนะนำเฉพาะบุคคล**:
${analysis.recommendations || 'การรับประทานอาหารให้สมดุลจะช่วยให้สุขภาพดีขึ้น'}

${analysis.meal_suggestions ? `🍽️ **แนะนำมื้อถัดไป**: ${analysis.meal_suggestions}` : ''}`;
        }
      } catch (error) {
      ...
      // มีรูปภาพแต่ไม่ต้องการวิเคราะห์
