...
export class HealthGoalService {
  ...
  async createHealthGoal(
    ...
    // สร้างเป้าหมายใหม่
    const healthGoal = this.healthGoalRepository.create({
      ...createHealthGoalDto,
      userId,
      start_date: new Date(createHealthGoalDto.start_date),
      target_date: createHealthGoalDto.target_date
        ? new Date(createHealthGoalDto.target_date)
        : null,
      current_value: createHealthGoalDto.current_value || 0,
    });

    const savedGoal = await this.healthGoalRepository.save(healthGoal);
    return this.mapToHealthGoalResponseDto(savedGoal);
  }
  ...
  async findAllHealthGoals(
    ...
    const queryBuilder = this.healthGoalRepository
    ...
    if (start_date_from) {
      queryBuilder.andWhere('goal.start_date >= :start_date_from', {
        start_date_from: new Date(start_date_from),
      });
    }

    if (start_date_to) {
      queryBuilder.andWhere('goal.start_date <= :start_date_to', {
        start_date_to: new Date(start_date_to),
      });
    }

    if (target_date_from) {
      queryBuilder.andWhere('goal.target_date >= :target_date_from', {
        target_date_from: new Date(target_date_from),
      });
    }

    if (target_date_to) {
      queryBuilder.andWhere('goal.target_date <= :target_date_to', {
        target_date_to: new Date(target_date_to),
      });
    }
  ...
  async updateHealthGoal(
    ...
    if (updateHealthGoalDto.start_date) {
      updateHealthGoalDto.start_date = new Date(updateHealthGoalDto.start_date);
    }

    if (updateHealthGoalDto.target_date) {
      updateHealthGoalDto.target_date = new Date(
        updateHealthGoalDto.target_date,
      );
    }
    ...
    Object.assign(goal, updateHealthGoalDto);
    const updatedGoal = await this.healthGoalRepository.save(goal);
  ...
  async deleteHealthGoal(
    ...
    await this.healthGoalRepository.remove(goal);
  ...
  async bulkUpdateGoals(
    ...
    const goals = await this.healthGoalRepository.find({
      where: { id: goal_ids, userId },
    });
    ...
    for (const goal of goals) {
      ...
      if (hasChanges) {
        await this.healthGoalRepository.save(goal);
        updatedCount++;
      }
    }
  ...
  async getGoalRecommendations(
    ...
    const user = await this.userRepository.findOne({
      where: { id: userId },
      relations: ['healthGoals', 'healthMetrics'],
    });
    ...
    if (!existingGoalTypes.includes(GoalType.EXERCISE)) {
      recommendations.push({
